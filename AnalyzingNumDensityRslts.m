function AnalyzingNumDensityRslts(Tooth1,Tooth2,RegType,Figures,CloseAll)
if nargin <= 4
    CloseAll = 'keep';
    if nargin <= 3
        Figures = 'off';
...
% logistic regression
if strcmp(RegType,'log') == 1
    for k1 = 1:size(ratiomat,2)
        [MyCoefs,xmodel,ymodel,Srtemp] = LogFit([1:10]',ratiomat(:,k1),'off');
        Sr(k1) = Srtemp;
        MyCoefs1(k1) = MyCoefs(1);
        MyCoefs2(k1) = MyCoefs(2);
        MyCoefs3(k1) = MyCoefs(3);
        MyCoefs4(k1) = MyCoefs(4);
        Xmodel{k1} = xmodel;
        Ymodel{k1} = ymodel;
    end
end
...
if strcmp(Figures,'on')
    ...
    title(['Num of Int. Teeth vs. Ratio for ' num2str(x(find(r2 == max(r2)))) ' NumDensity (NumDensity with best r^2)'])
    xlabel('Num of Int. Teeth')
    ylabel('Ratio')
    if strcmp(RegType,'linear')
...
if strcmp(RegType,'linear') == 1
    ...
    load([base_path 'Topr2Rank.mat'])
    Topr2Rank(loc1,loc2) = -(r2ordered(1));
    Topr2Rank(loc2,loc1) = Topr2Rank(loc1,loc2);
    save([base_path 'Topr2Rank.mat'],'Topr2Rank')
    ...
    Srordered = sort(Sr);
    SrRank = zeros(1,length(Sr));
    for k1 = 1:length(Sr)
        check = x(find(Sr == Srordered(k1)));
        if size(check,2) > 1
            k3 = 1;
            while isempty(find(SrRank(1:k1-1) == check(k3))) == 0
                k3 = k3 + 1;
            end
            SrRank(k1) = check(k3);
        else
            SrRank(k1) = x(find(Sr == Srordered(k1)));
        end
    end
    load([base_path 'TopCoefs1Rank.mat'])
    load([base_path 'TopCoefs2Rank.mat'])
    load([base_path 'TopCoefs3Rank.mat'])
    load([base_path 'TopCoefs4Rank.mat'])
    load([base_path 'DiffLogRank.mat'])
    ...
    TopCoefs1Rank(loc1,loc2) = MyCoefs1(Sample);
    TopCoefs1Rank(loc2,loc1) = TopCoefs1Rank(loc1,loc2);
    save([base_path 'TopCoefs1Rank.mat'],'TopCoefs1Rank')
    
    TopCoefs2Rank(loc1,loc2) = MyCoefs2(Sample);
    TopCoefs2Rank(loc2,loc1) = TopCoefs2Rank(loc1,loc2);
    save([base_path 'TopCoefs2Rank.mat'],'TopCoefs2Rank')
    
    TopCoefs3Rank(loc1,loc2) = MyCoefs3(Sample);
    TopCoefs3Rank(loc2,loc1) = TopCoefs3Rank(loc1,loc2);
    save([base_path 'TopCoefs3Rank.mat'],'TopCoefs3Rank')
    
    TopCoefs4Rank(loc1,loc2) = MyCoefs4(Sample);
    TopCoefs4Rank(loc2,loc1) = TopCoefs4Rank(loc1,loc2);
    save([base_path 'TopCoefs4Rank.mat'],'TopCoefs4Rank')
    
    DiffLogRank(loc1,loc2) = abs(Ymodel{Sample}(end) - Ymodel{Sample}(1));
    DiffLogRank(loc2,loc1) = DiffLogRank(loc1,loc2);
    save([base_path 'DiffLogRank.mat'],'DiffLogRank')
    
    load('C:/Users/Kevin/Documents/MATLAB/cPdist-master/DistRslts0Sample/cPDistMatrix.mat')
    %if cPDistMatrix(loc1,loc2) < 0.02
        figure;plot([1:10]',ratiomat(:,Sample),'ko',Xmodel{Sample},Ymodel{Sample},'-k')
        title(num2str(cPDistMatrix(loc1,loc2)))
    %end
